...
int main(int argc, char *argv[])
{
	...
	/* CHECK THE TARGET VARIABLE, MAY BE ABLE TO GET RID OF IT!!! */
	char *title = strdup(argv[2]);  //The 3rd argument has to be the 'target'. !!this was target testing title instead!!
	FILE *landing_page;
	landing_page = fopen(argv[3], "r");  //The 4th argument has to be the 'landing_page'.
	int strs = strcnt(string, ",") + 1;  //Gets the number of strings to store.
	char *container[strs];
